		...
		if ((detectIsHand(img))) {			
			...
			if (defectIdAfter.size() > 0) {
				...
				for (int j = startId; ; j++){
					...
					Core.circle(img, curDefectPoint, 2, new Scalar(0, 0, 0), -5);
			...
			for (int fid = 0; fid < finTipsTemp.size(); ){
				...
				Core.line(img, inCircle, curFinPoint, new Scalar(0, 255, 255), 2);
				Core.circle(img, curFinPoint, 2, Scalar.all(0), -5);
				
				Core.putText(img, Integer.toString(count), new Point(curFinPoint.x - 10, 
									curFinPoint.y - 10), Core.FONT_HERSHEY_SIMPLEX, 0.8, Scalar.all(0));
